...
class PointPillarGmatchLoss(PointPillarDepthLoss):
    def __init__(self, args):
        super(PointPillarGmatchLoss, self).__init__(args)
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        output_dict_S = {'cls_preds': output_dict['cls_preds_S'],
                         'reg_preds': output_dict['reg_preds_S'],
                         'dir_preds': output_dict['dir_preds_S']}
        output_dict_T = {'cls_preds': output_dict['cls_preds_T'],
                         'reg_preds': output_dict['reg_preds_T'],
                         'dir_preds': output_dict['dir_preds_T']}
        
        loss_T = super().forward(output_dict_T, target_dict, suffix)
        loss_S = super().forward(output_dict_S, target_dict, suffix)
        ...
        self.loss_dict.update({'loss_S': loss_S.item(),
                                'loss_T': loss_T.item(),
                               'generate_loss': generate_loss.item()})

        return loss_S, loss_T, generate_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix="", iter=None):
        ...
        loss_S = self.loss_dict.get('loss_S', 0)
        ...
        print("[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"
              " || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || Depth Loss: %.4f || Gen Loss: %.4f" % (
                  ...
                  loss_S, cls_loss, reg_loss, dir_loss, iou_loss, depth_loss, gen_loss))
        ...
        wandb.log({ 'loss_S'+suffix: loss_S,
