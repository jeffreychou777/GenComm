...
class PointPillarDiffcommMessageLoss(PointPillarDepthLoss):
    def __init__(self, args):
        super(PointPillarDiffcommMessageLoss, self).__init__(args)
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        message = output_dict['message'].permute(0, 2, 3, 1).contiguous() \
                    .view(batch_size, -1,  1)
        cls_weights = positives * self.pos_cls_weight + negatives * 1.0
        cls_weights /= torch.clamp(pos_normalizer, min=1.0)
        message_loss = sigmoid_focal_loss(message, cls_labls, weights=cls_weights, **self.cls)
        message_loss = message_loss.sum() * self.cls['weight'] / batch_size
        
        
        self.loss_dict.update({'generate_loss': generate_loss.item()})
        self.loss_dict.update({'message_loss': message_loss.item()})

        total_loss = total_loss + self.generate_weight * generate_loss + message_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix="", iter=None):
        ...
        mes_loss = self.loss_dict.get('message_loss', 0)
        ...
        wandb.log({ 'train_loss_iter'+suffix: total_loss,
                    ...
                    'Mes_loss'+suffix: mes_loss}, step=iter,
                    )
