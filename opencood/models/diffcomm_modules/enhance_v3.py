...
class WindowAttention_sparse(nn.Module):
    def __init__(self, dim, win_size,num_heads, token_projection='linear', qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # 新增: ego-relative位置编码
        self.ego_dist_bins = 3  # 距离bins数量
        self.ego_angle_bins = 3  # 角度bins数量 
        
        # ego相对位置编码表
        self.ego_position_bias_table = nn.Parameter(
            torch.rand(self.ego_dist_bins * self.ego_angle_bins, num_heads))
    ...
    def get_ego_relative_bias(self, x_windows, affine_matrix, H=None, W=None):
        ...
        ego_relative_bias = F.embedding(position_index, self.ego_position_bias_table)
...
class Enhancerv3(nn.Module):
    '''
    v3 is multi-scale with attn1 ** 2, 3 bins and rand init
    '''
    def __init__(self, C, win_size, num_heads):
        super().__init__()
