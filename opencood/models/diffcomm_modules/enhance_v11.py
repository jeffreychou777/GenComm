...
class SplitAttn(nn.Module):
    ...
    def forward(self, window_list, affine_matrix=None):
        ...
        angle_bias = self.get_angle_bias(sw, affine_matrix, H=sw.shape[1], W=sw.shape[2])
        distance_bias = self.get_distance_bias(sw, affine_matrix, H=sw.shape[1], W=sw.shape[2])
        self.visualize_angle_bias(angle_bias, save_path='/home/junfei.zhou/DATACENTER2/data/code/DiffComm/bias_vis/angle_bias_both.png')
        self.visualize_angle_bias(distance_bias, save_path='/home/junfei.zhou/DATACENTER2/data/code/DiffComm/bias_vis/distance_bias_both.png')

        out = sw * x_attn[:, :, :, 0:self.input_dim] + \
              mw * x_attn[:, :, :, self.input_dim:2*self.input_dim] * angle_bias +\
              bw * x_attn[:, :, :, self.input_dim*2:] * distance_bias
...
class Enhancerv11(nn.Module):
    '''
    v11 is v7 add distance and angle bias in split_attn 
    '''
    def __init__(self, C, win_size, num_heads):
        super().__init__()
...
def test_sparse():
    ...
    block = Enhancerv11(C, win_size, num_heads,)
