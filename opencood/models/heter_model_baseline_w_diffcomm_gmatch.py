...
from opencood.models.diffcomm_modules.sparse_attn import MessageExtractor
from opencood.models.diffcomm_modules.enhance import Enhancer
from opencood.models.diffcomm_modules.enhance import EnhancerWoMs
from opencood.utils.transformation_utils import normalize_pairwise_tfm
from opencood.utils.model_utils import check_trainable_module, fix_bn, unfix_bn
import importlib
import torchvision

class HeterModelBaselineWDiffCommGmatch(nn.Module):
    def __init__(self, args):
        super(HeterModelBaselineWDiffCommGmatch, self).__init__()
        self.args = args
        ...
        if 'message_extractor' in args and args['message_extractor'] == 'enhance':
            self.message_extractor = MessageExtractor(128, [8,8], 4,)
            print('message_extractor: enhance')
        ...
        if 'enhancer' in args and args['enhancer'] == 'enhance':
            ...
            print("use enhancer")
        if 'enhancer' in args and args['enhancer'] == 'enhance_woms':
            self.enhancer = EnhancerWoMs(128, [8, 8], 4)
            print("use enhance_woms")
    ...
    def forward(self, data_dict):
        ...
        pred_feature = self.diffcomm(heter_feature_2d, conditions, record_len)['pred_feature']
        heter_feature_2d = pred_feature
        
        if len(pred_feature.shape) == 3:
            pred_feature = pred_feature.unsqueeze(0) ## for the case of bs=1 and only ego
        ...
        # fused_feature_S = self.fusion_net(gt_feature, record_len, affine_matrix)
        fused_feature_S = self.fusion_net(heter_feature_2d, record_len, affine_matrix)
        fused_feature_T = self.fusion_net(gt_feature, record_len, affine_matrix)

        if self.shrink_flag:
            fused_feature = self.shrink_conv(fused_feature)

        cls_preds_S = self.cls_head(fused_feature_S)
        reg_preds_S = self.reg_head(fused_feature_S)
        dir_preds_S = self.dir_head(fused_feature_S)
        ...
        output_dict.update({'cls_preds_S': cls_preds_S,
                            'reg_preds_S': reg_preds_S,
                            'dir_preds_S': dir_preds_S,
                            'cls_preds_T': cls_preds_T,
                            'reg_preds_T': reg_preds_T,
                            'dir_preds_T': dir_preds_T,
                            'cls_preds': cls_preds_S,
                            'reg_preds': reg_preds_S,
                            'dir_preds': dir_preds_S,
